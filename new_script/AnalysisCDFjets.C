...
taskjets = AddTaskJets("AOD","FASTJET", 0.7, kHighPtFilterMask, 0.15, 0. ); // no background subtraction

if ( kDeltaAODJetName.Length() > 0 ) { taskjets->SetNonStdOutputFile(kDeltaAODJetName.Data()); }

kNonStdBranchInput = taskjets->GetNonStdBranch();

if ( kNonStdBranchInput.Length() ) { kJetListSpectrum.Add(new TObjString(kNonStdBranchInput.Data())); }

// cout << "JETFINDER :: nonstdfile : "   << taskjets->GetNonStdFile() << endl; // no getter in AliAnalysisTaskJets
cout << "JETFINDER :: nonstdbranch : " << kNonStdBranchInput.Data() << endl;
