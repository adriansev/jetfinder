...
void runEMCalJetAnalysis(
         ...
         UInt_t         numLocalFiles       = 130,                          // number of files analyzed locally
         ...
         )
{
  ...
  TString   kAnalysisMode = "local" ; kAnalysisMode.ToLower();  // local, grid, proof
  ...
  if (useTender)
    ...
    AliAnalysisTaskSE *tender = AddTaskEMCALTender ( runPeriod, kFALSE, kTRUE, kTRUE, kTRUE, kTRUE, kTRUE, kFALSE, kTRUE, kTRUE, kTRUE, kFALSE, AliEMCALRecParam::kClusterizerNxN );
  ...
  //  const char *tag            = "Jet"
  //  )
  AliEmcalJetTask* jetFinderTask = AddTaskEmcalJet( AliEmcalJetTask::kAKT, "Tracks", "CaloClusters", 0.15, 0.30, 0.005, 0.2, "Jet02");
...
//  AliEmcalJetTask* jetFinderTask = AddTaskEmcalJet( AliEmcalJetTask::kAKT, "Tracks", "CaloClusters", 0.15, 0.30, 0.005, 0.6, "Jet06");
  ...
  //  Int_t       leadhadtype        = 0,
  //  const char *taskname           = "AliAnalysisTaskEmcalJetCDF"
  //  )
  AliAnalysisTaskEmcalJetCDF* anaTask = AddTaskEmcalJetCDF("Tracks", "CaloClusters", jetFinderTask->GetName(), "");
...
//     mgr->StartAnalysis("local", chain);
    mgr->StartAnalysis( kAnalysisMode.Data(),chain);
                                     ...
                                     Int_t workerTTL, Bool_t isMC)
{
  ...
  plugin->SetROOTVersion("v5-34-10-1");
  plugin->SetAliROOTVersion("v5-05-20-AN");
