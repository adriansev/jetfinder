...
//     Int_t       leadhadtype        = 0;                // replaced default value -> already set up
    ...
    Double_t jetpt_cuts[] = {1., 5., 10. ,20.};

    for ( std::vector<TString>::iterator jf_it = jf_names.begin(); jf_it != jf_names.end(); ++jf_it) // loop over all jet finders
        {
        for (UInt_t k = 0; k < sizeof(jetpt_cuts)/sizeof(Double_t); ++k )  // loop over all jet pt cuts
            ...
            taskname = Form ("Sample%i", k);

            anaTask = AddTaskEmcalJetSample( tracksName.Data() , clustersCorrName.Data(), jf->GetJetsName(), jf->GetRhoName(), nCentBins, jf->GetRadius(),
                                             jetpt_cuts[k], jetareacut, acceptance_type.Data(), leadhadtype, taskname);
